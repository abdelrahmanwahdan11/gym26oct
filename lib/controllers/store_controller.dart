...
class StoreController extends ChangeNotifier {
  ...
  Future<Product?> resolveProduct(String id) async {
    try {
      return _products.firstWhere((element) => element.id == id, orElse: () => throw StateError('not found'));
    } catch (_) {
      final fetched = await repository.findProduct(id);
      if (fetched != null && _products.every((element) => element.id != fetched.id)) {
